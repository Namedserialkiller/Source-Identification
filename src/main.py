...
def run_pipeline(
    ...
    if use_live_data:
        ...
        try:
            df = fetch_delhi_aqi(hours_back=168, use_historical_fallback=True)
        ...
        # For live data, load existing model and predict
        from predict import predict, predict_proba
        from evaluate import load_model_and_encoder
        model, encoder = load_model_and_encoder()
        predictions = predict(df)
        probabilities = predict_proba(df)
        
        print("=== Live Pollution Source Predictions ===")
        for i, (pred, probs) in enumerate(zip(predictions, probabilities)):
            print(f"Record {i+1}: {pred.upper()} (confidence: {probs.max():.2%})")
            top3 = sorted(zip(encoder.classes_, probs), key=lambda x: x[1], reverse=True)[:3]
            print(f"  Top 3: {', '.join(f'{cls}: {prob:.1%}' for cls, prob in top3)}")
        return df
